 ...
 */
const parsePageMd = (md, gitLog, rootDir, mdPath, destFromSeverRoot) => {
  ...
  const result = {
    ...
    createdAuthorName: gitLog.created ? gitLog.created.name : '',
    createdDate: gitLog.created ? gitLog.created.date : '',
  }
  if (destFromSeverRoot) {
    result.fromRoot = destFromSeverRoot + '/' + result.fromRoot
    result.toRoot = destFromSeverRoot.split('/').map(() => '../') + result.toRoot
  }
