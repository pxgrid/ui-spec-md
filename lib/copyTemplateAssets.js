...
const copyTemplateAssets = function ( stream ) {
	...
	// fs.createReadStream( __dirname + '/template/build/_template/_template.woff' ).pipe( fs.createWriteStream( '_template/_template.woff' ) );
	var fontSrc = fs.readFileSync( __dirname + '/template/build/_template/_template.woff' );
	let fontFile = new File( {
		path: '_template/_template.woff',
		contents: new Buffer( fontSrc )
	} );
	stream.push( fontFile );
